...
两条线路不同：**上传**直接进入微软的存储服务，通常能跑满带宽；**下载**要从微软的文件分发站取回内容，这段线路的质量由你的网络环境决定，插件无法控制。蜂窝弱信号、晚间高峰或运营商线路不佳时，下载可能只有几十 KB/s 甚至间歇失败——这不代表插件出了问题。中断会从断点继续、太慢的连接会自动更换，不会从头重来。

可以按下面的顺序自查：

1. **换个 DNS 试试**（最常见有效）。DNS 就是“查门牌”的服务：实测中同一时刻，默认 DNS 查不到微软分发站的地址、换公共 DNS 就能查到的情况确实存在。手机在 Wi-Fi 设置里修改（iPhone 为“配置 DNS”，安卓为“私有 DNS”），电脑在网络设置或路由器里修改。推荐值：国内 223.5.5.5（阿里）或 119.29.29.29（腾讯）；海外 1.1.1.1 或 8.8.8.8。
2. **检查代理、加速器与防火墙**。确认分流或拦截规则没有漏掉、挡下微软下载域名 `my.microsoftpersonalcontent.com`；某些网络下，让这个域名走代理反而更稳。
3. **换网络或换时间再同步**。弱信号蜂窝和晚间高峰的下载速度可能只有平时的几分之一，换个 Wi-Fi 或避开高峰常常自己就恢复了。
4. **（电脑端、有经验用户）修改 hosts**：为 `my.microsoftpersonalcontent.com` 指定一个可达的 IP。此法有门槛且可能随微软调整失效，仅作最后手段。

都试过仍然长期很慢，请按第 8 节反馈并附上诊断报告——它能帮着分清是线路问题还是插件问题。
