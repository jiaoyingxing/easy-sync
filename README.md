...
1. **换 DNS**：手机在 Wi-Fi 设置里改（iPhone 为“配置 DNS”，安卓为“私有 DNS”），电脑在网络设置或路由器里改。国内推荐 223.5.5.5（阿里）或 119.29.29.29（腾讯），海外推荐 1.1.1.1 或 8.8.8.8——默认 DNS 解析不到分发站时，换公共 DNS 常常就能解决。
2. **检查代理与防火墙**：确认规则没有漏掉或拦下微软下载域名 `my.microsoftpersonalcontent.com`；某些网络下让它走代理反而更稳。
3. **换网络或错峰再同步**：换个 Wi-Fi 或避开高峰，常常自己就恢复了。
4. **（电脑端）修改 hosts**：把 `my.microsoftpersonalcontent.com` 指向可达的 IP。有门槛、可能随微软调整失效，仅作最后手段。
