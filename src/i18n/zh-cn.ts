...
const zhCN: LocaleStrings = {
  ...
  "settings.communityPlugins.status.manifestIssue": "本机插件信息异常",
  ...
  "settings.communityPlugins.status.localBundleIncomplete": "本机插件文件不完整，请重新安装插件后同步",
  ...
  "settings.communityPlugins.decisions.open": "选择“{plugin}”的启用状态",
  "settings.communityPlugins.decisions.title": "选择“{plugin}”的启用状态",
  "settings.communityPlugins.decisions.message": "本机为“{local}”，远端为“{remote}”。请选择同步后保留哪一边。这个选择不会删除插件文件。",
  ...
  "notice.communityPlugins.decisionUnavailable": "这项待确认状态已失效，请重新同步后再处理。",
  ...
  "syncPlan.migrationSummary": "这台设备需要完成一次同步升级。确认后，将按下方计划继续同步。",
  "syncPlan.cloudJoinSummary": "这台设备正在加入已有同步状态。确认计划后，EasySync 会先建立本机同步状态，再按计划同步文件。",
  "syncPlan.confirmMigration": "继续升级并同步",
  "syncPlan.migrationConfirmTitle": "继续升级并同步？",
  "syncPlan.migrationConfirmMessage": "EasySync 会先完成这台设备的同步升级，再按侧栏计划同步文件。仍在使用旧版 EasySync 的设备若之后再次同步，可能出现冲突或待处理项，建议尽快更新这些设备。取消不会修改本地或云端文件。",
  ...
  "syncView.communityPlugins.pendingDescription": "有 {count} 个插件在本机和远端的启用状态不同，请选择同步后保留本机还是远端状态。",
